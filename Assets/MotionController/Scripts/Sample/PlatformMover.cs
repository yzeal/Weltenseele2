    ...
    void InternalUpdate()
    {
        if (Move)
        {
            ...
            if (rigidbody != null)
            {
                ...
                {
                    rigidbody.MovePosition(transform.position + (mVelocity * Time.deltaTime));
                }
                else
                {
                    rigidbody.MovePosition(transform.position + (mVelocity * Time.deltaTime));
        ...
        if (Rotate)
        {
            if (rigidbody != null)
            {
                if (rigidbody.isKinematic)
                {
                    rigidbody.MoveRotation(transform.rotation * Quaternion.AngleAxis(45f * Time.deltaTime, Vector3.up));
                }
                else
                {
                    rigidbody.MoveRotation(transform.rotation * Quaternion.AngleAxis(45f * Time.deltaTime, Vector3.up));
